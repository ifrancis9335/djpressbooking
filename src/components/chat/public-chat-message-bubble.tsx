...
export function PublicChatMessageBubble({ role, content, intent }: PublicChatMessageBubbleProps) {
  ...
  return (
    <article className={cn("flex", assistant ? "justify-start" : "justify-end")}>
      <div
        className={cn(
          "max-w-[90%] min-w-0 rounded-2xl border px-3 py-2.5 text-sm shadow-panel",
          assistant
      ...
      >
        ...
        <p className="whitespace-pre-wrap break-words text-[13px] leading-5 text-inherit">{content}</p>
      </div>
    </article>
  );
}